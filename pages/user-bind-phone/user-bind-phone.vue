<template>
	<view class="body">
		<input type="number" v-model="phone" class="uni-input common-input" placeholder="输入手机号" :disabled="isbind" />
		...
		<button class="user-set-btn" :loading="loading" :class="{'user-set-btn-disable':disabled}" type="primary" @tap="submit"
		 :disabled="disabled || isbind">{{isbind?"已绑定":"立即绑定"}}</button>
	...
	export default {
		...
		methods: {
			...
			async submit() {
				...
				let pic=uni.getStorageSync('userinfo').userpic
