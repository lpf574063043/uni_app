<template>
	<view>
		<view class="iconfont icon-zengjia" style="position: absolute;right: 15upx;top: 15upx;z-index: 1000;color: #FFFFFF;background: rgba(51, 51, 51, 0.4);border-radius: 100%;padding: 20upx;font-size: 32upx;line-height: 1;"
		 @tap="togleShow"></view>
		<!-- 背景图 -->
		<user-space-head @update="updateGuanZhu" @changeimg="changeimg" :unserinfo="userinfo" />
		<!-- 统计 -->
		...
		<block v-for="(item,index) in tablist" :key="index">
			...
			<template v-else-if="tabIndex==index">
				...
				<template v-else>
					<!-- 无内容默认 -->
					<view style="font-size: 50upx;font-weight: bold;color: #CCCCCC;
							padding-top: 100upx;" class="u-f-ajc">No
						content~</view>
				</template>
			</template>
		</block>
		...
		<!-- 操作菜单 -->
		<user-space-popup :show="show" :isblack="userinfo.isblack" @hide="togleShow" @lahei="lahei" @chat="goToChat"></user-space-popup>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			async getUserInfo(userid) {
				...
				let regtime = info.create_time ? Time.gettime.dateFormat(new Date(info.create_time * 1000), '{Y}-{MM}-{DD}') :
					"未知";
				this.userinfo = {
					isme: isme,
					bgimg: 1,
			...
			async getList() {
				...
				let url = this.userinfo.isme ? `/user/post/${page}` : `/user/${this.userinfo.id}/post/${page}`;//this.userinfo.isme ?判断查看个人信息是自己还是别人
